...
cli.responders.moreUserInfo = function(str) {
    ...
    if (str) {
        ...
        const userId = helpers.isTypeOfValid(splitArr[1], 'string') && splitArr[1].trim() > 0 ? splitArr[1].trim() : false;

        console.log({ userId, splitArr });
        ...
        if (userId) {
            _data.read('users', userId, function(err, userData) {
                if (!err && userData) {
                    ...
                    console.log(`User with id ${str} not found`);
                }
            })
        } else {
        ...
        if (!err && checkIds) {
            ...
            checkIds.forEach(function(id) {
                _data.read('checks', id, function(err, checkData) {
                    if (!err && checkData) {
                        let line = `Name: ${checkData.firstName} ${checkData.lastName} Phone: ${checkData.phone} Checks: `;
                        const checks = helpers.isTypeOfValid(checkData.checks, 'object') && helpers.isInstanceOfArray(checkData.checks) && checkData.checks.length ? userData.checks.length : 0;
