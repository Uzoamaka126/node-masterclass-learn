...
// get more user info: 'more user info --{userId}'
...
// request for more check info: 'more check info --{checkId}'
...
// request for more log info: 'more log info --{fileName}'
cli.responders.moreLogInfo = function(str) {
    if (str) {
        // get the log file name from the CLI command string
        const splitArr = str.split("--");
        const logFileName = helpers.isTypeOfValid(splitArr[1], 'string') && splitArr[1].trim().length > 0 ? splitArr[1].trim() : false;

        if (logFileName) {
            cli.verticalSpace();

            _logs.decompress(logFileName, function(err, data) {
                if (!err && data) {
                    // split into lines
                    const arr = data.split('\n');
                    // still individual JSON strings
            ...
            console.log('Pass the log file name as a flag');
        }
    }
};
