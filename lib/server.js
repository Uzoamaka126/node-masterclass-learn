...
server.unifiedServer = function(req, res) {
    ...
    req.on('end', function() {
        ...
        // let selectedRequestHandler = typeof(server.router[trimmedPath]) !== 'undefined' ? server.router[trimmedPath] : server.router['404'];
        ...
        try {
            selectedRequestHandler(data, function(statusCode, payload, contentType) {
                server.processSelectedRequestHandler(res, trimmedPath, method, statusCode, payload, contentType)
            });
        } catch (error) {
            debug(error);
            server.processSelectedRequestHandler(res, trimmedPath, method, 500, {'Error': 'Unknown error' }, 'json')
        }
    });
};

server.processSelectedRequestHandler = function (res, method, trimmedPath, statusCode, payload, contentType) {
