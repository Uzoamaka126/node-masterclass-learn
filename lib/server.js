...
server.unifiedServer = function(req, res) {
    ...
    req.on('end', function() {
        ...
        console.log({ 'server.router': server.router, trimmedPath });

        let computeRequestHandler = '';

        const serverRouterObjKeys = Object.keys(server.router);

        for (var i = 0; i < serverRouterObjKeys.length; i++) {
            if (trimmedPath.includes(serverRouterObjKeys[i])) {
                computeRequestHandler = server.router[serverRouterObjKeys[i]]
            } else {
                computeRequestHandler = server.router['404']
            }
        }
